...
function oSearchSuggest(searchFuc){  
    ...
    this.dataDisplay = function(data){  
        ...
        for(var i=0; i<data.length; i++){  
            ...
            li.innerHTML = data[i].split(':')[1];
            li.setAttribute('data-value',data[i].split(':')[0]);
            //li.innerHTML += '<input type="hidden" class="hid" value="'+data[i].split(':')[0]+'">';
...
function sendKeyWordToBack(keyword) {
    ...
    $.ajax({
        ...
        url: "/broadband/autodeal/BroadbandAddress",
        async: false,
        data: obj,
        dataType: "json",
        success: function (data) {
            //var json = eval("("+data+")");
            var key = data.data.split(",");
            var aData = [];
            for (var i = 0; i < key.length; i++) {
                //以下为根据输入返回搜索结果的模拟效果代码,实际数据由后台返回
                if (key[i] != "") {
                    aData.push(key[i].replace(/"/g,''));
                }
            }
